...
import { X, Heart, CheckCircle2, Minus, Plus, ShoppingBag, ChevronLeft, ChevronRight, Maximize2 } from 'lucide-react';
    ...
    <>
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-6">
        ...
        <div className="relative bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col md:flex-row animate-in fade-in zoom-in-95 duration-200 max-h-[90vh]">
          ...
          >
            ...
            <button onClick={(e) => { e.stopPropagation(); fechar(); }} className="md:hidden absolute top-4 right-4 p-2 bg-white/80 backdrop-blur rounded-full text-stone-800 hover:bg-white active:scale-90 z-20">
              <X className="w-5 h-5" />
            </button>
      ...
      {isZoomed && (
        ...
        >
          ...
          <div 
            className="w-full h-full flex items-center justify-center relative outline-none"
            onTouchStart={onTouchStartEvent}
            onTouchMove={onTouchMoveEvent}
            onTouchEnd={onTouchEndEvent}
          >
            ...
            {galeria.length > 1 && (
              <>
                <button 
                  onClick={(e) => imagemAnterior(e)}
                  className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-black/50 hover:bg-black/80 rounded-full text-white transition-all active:scale-90 hidden sm:block"
                >
                  <ChevronLeft className="w-8 h-8" />
                </button>
                <button 
                  onClick={(e) => proximaImagem(e)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-black/50 hover:bg-black/80 rounded-full text-white transition-all active:scale-90 hidden sm:block"
                >
                ...
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                  {galeria.map((_, idx) => (
                    <div 
                      key={idx} 
                      className={`w-2.5 h-2.5 rounded-full transition-all shadow-sm ${idx === imagemAtualIndex ? 'bg-white w-6' : 'bg-white/40'}`}
                    />
                  ))}
                </div>
              </>
